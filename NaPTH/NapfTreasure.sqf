private ["_positions","_num","_Monitor","_fCrate","_area","_dots","_point"];
	...
	while {_visited} do {
		sleep 1;
		{
			if((isPlayer _x) && (_x distance _crate <= 3)) then {
				_visited = false;
				diag_log format["[NaPTH]: %1 has cleard crate at %2",_x call fa_plr2str,mapGridPosition _crate];
			};
		} forEach playableUnits;
	};
...
_fCrate = {
	...
	diag_log format["[NaPTH]: Created Crate at %1 on Napf Island",mapGridPosition _this];
};
...
ST_count = 0;
_positions = selectBestPlaces [_point, 400, "forest + meadow + trees * 0.5 - houses", 10, _num];
{
	private ["_crate","_objectID"];
	diag_log "[NaPTH]: Spawning a crate";

	//Spawn crate
	_crate = ST_crate_Type createVehicle (_x select 0);
...
while {ST_marker_island && (ST_count < _num)} do {
	_area = createMarker ["SkalistyCrate", _point];
	_area setMarkerColor "ColorBlue";
	_area setMarkerShape "ELLIPSE";
	_area setMarkerBrush "Horizontal";
	_area setMarkerSize [400, 400];
	_dots = createMarker ["SkalistyCrate_dot", _point];
...
diag_log "[NaPTH]: All crates are found";
